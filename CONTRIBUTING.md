...
| [configdirector_openfeature_flutter_provider](packages/configdirector_openfeature_flutter_provider) | OpenFeature provider wrapping the client SDK, in beta: see below      |

The OpenFeature provider builds on `openfeature_dart_client_sdk`, the static-context client SDK,
which is in beta. Until that goes stable the provider is published as a pre-release, with a new
beta for each OpenFeature beta it is built against. It depends on the *published* client SDK, the
way a consumer does, and needs Dart 3.12.2, so Flutter 3.44.2 or later.
