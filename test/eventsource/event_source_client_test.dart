...
http.StreamedResponse _sseResponse(List<String> chunks, {int statusCode = 200}) {
...
void main() {
  group('EventSourceClient', () {
    group('request configuration', () {
      ...
      test('reports a redirect status as an error when followRedirects is false', () async {
        final client = MockClient.streaming((request, bodyStream) async {
    ...
    group('readyState', () {
      test('starts as CLOSED', () {
        ...
        final source = EventSourceClient(url: Uri.parse('http://localhost/sse'), client: client);
      ...
      test('becomes OPEN once the response headers arrive, then notifies listeners', () async {
        final client = MockClient.streaming((request, bodyStream) async {
          return _sseResponse(['data: hi\n\n']);
        });
      ...
      test('does not fire an extra notification when close() is called explicitly on an open connection', () async {
        final client = MockClient.streaming((request, bodyStream) async {
          return http.StreamedResponse(_neverEndingStream(), 200);
        });
        ...
        expect(notificationsAfterClose, 1); // exactly the open -> closed transition
      });
    });
    ...
    group('messages', () {
      ...
      test('delivers events to multiple listeners (broadcast stream)', () async {
        final client = MockClient.streaming((request, bodyStream) async {
    ...
    group('errors', () {
      ...
      test('emits ValueOutOfRangeError when calculateReconnectDelay returns an out-of-range value', () async {
        final client = MockClient.streaming((request, bodyStream) async {
        ...
        final source = EventSourceClient(
          url: Uri.parse('http://localhost/sse'),
          client: client,
          calculateReconnectDelay: (_) => Duration.zero, // below the minimum of 1 ms
    ...
    group('reconnection', () {
      ...
      test('passes serverReconnectionTime from SSE retry field to calculateReconnectDelay', () async {
        ...
        expect(capturedServerReconnectionTime, const Duration(milliseconds: 5000));
      });
      ...
      test('passes reconnection state to calculateReconnectDelay', () async {
        ...
        expect(capturedState?.serverReconnectionTime, const Duration(seconds: 2));
      });
      ...
      for (final entry in <String, Duration>{
        ...
        test('emits an error and falls back to serverReconnectionTime when delay is ${entry.key}', () async {
          final client = MockClient.streaming((request, bodyStream) async {
            return _sseResponse(['retry: 5\ndata: hi\n\n']);
          });
      ...
      for (final entry in <String, Duration>{
        ...
        test('does not emit an error when delay is at the valid boundary: ${entry.key}', () async {
          final client = MockClient.streaming((request, bodyStream) async {
            return _sseResponse(['data: hi\n\n']);
          });
